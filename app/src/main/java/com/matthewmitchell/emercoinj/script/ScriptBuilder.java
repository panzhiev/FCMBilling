...
package com.matthewmitchell.emercoinj.script;

import com.google.common.collect.Lists;
import com.matthewmitchell.emercoinj.core.Address;
import com.matthewmitchell.emercoinj.core.ECKey;
import com.matthewmitchell.emercoinj.core.Utils;
import com.matthewmitchell.emercoinj.crypto.TransactionSignature;
import com.matthewmitchell.emercoinj.utils.NvsScriptUtils;
...
import static com.matthewmitchell.emercoinj.script.ScriptOpCodes.*;

/**
 * <p>Tools for the construction of commonly used script types. You don't normally need this as it's hidden behind
 * convenience methods on {@link com.matthewmitchell.emercoinj.core.Transaction}, but they are useful when working with the
...
public class ScriptBuilder {
    ...
    /** Creates a scriptPubKey for new NVS record. */
    public static Script createNewNvsScript(String name, String value, String days, Address address){

        ScriptBuilder scriptBuilder = new ScriptBuilder();

        // OP_1 OP_DROP <name> <days> OP_2DROP <value> OP_DROP OP_DUP OP_HASH160 <pubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
        scriptBuilder
                .op(ScriptOpCodes.OP_1)
                .op(ScriptOpCodes.OP_DROP)
                .data(name.getBytes())
                .data(NvsScriptUtils.getBytesFromDays(days))
                .op(ScriptOpCodes.OP_2DROP);

        // split value in 520 bytes chunks and add it to script
        byte[][] valueChunks = NvsScriptUtils.divideValue(value.getBytes());
        for (byte[] valueChunk : valueChunks) {
            scriptBuilder.data(NvsScriptUtils.removeZeroBytes(valueChunk));
        }
        //insert end opcodes
        for (int i = 0; i < valueChunks.length / 2; i++) {
            scriptBuilder.op(ScriptOpCodes.OP_2DROP);
        }
        if (valueChunks.length % 2 != 0) {
            scriptBuilder.op(ScriptOpCodes.OP_DROP);
        }

        //insert the rest of script data and opcodes
        // OP_DUP OP_HASH160 <pubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
        scriptBuilder
    ...
    /** Creates a scriptPubKey for update NVS record. */
    public static Script createUpdateNvsScript(String name, String value, String days, Address address){
    ...
    /** Creates a scriptPubKey for delete NVS record. */
    public static Script createDeleteNvsScript(String name, Address address){

        // OP_3 OP_DROP <name> OP_DROP OP_DUP OP_HASH160 <pubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
        return new ScriptBuilder()
                .op(ScriptOpCodes.OP_3)
                .op(ScriptOpCodes.OP_DROP)
                .data(name.getBytes())
                .op(ScriptOpCodes.OP_DROP)
                .op(ScriptOpCodes.OP_DUP)
